...
public class Baek_1107 {
    ...
    public static void main(String[]args) throws IOException {
        ...
        int num=remocon(arr,to,0,to);
        System.out.println(Math.min(Math.abs(to-100),num));

    }
    //사용하지 말아야 할 수 배열, 가야할 채녈, 반환값
    static int remocon(int[]arr,int to,int val,int min) {
        //가야할 채널 자리수+1 == 우리가 얻은 채널 자리수면 반환
        if (Integer.toString(val).length() == Integer.toString(to).length()+1) {
            return min;
        }
